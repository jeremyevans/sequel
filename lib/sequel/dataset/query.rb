...
module Sequel
  class Dataset
    ...
    #
    #   DB[:items].select(:a, :b).replace_select_with_alias(Sequel.as("1", :a)) # SELECT b, '1' AS a FROM items
    ...
    #   DB[:items].selectt(:a, :b).replace_select_with_alias { |o| Sequel.as("1", o.a) }.sql.should == "SELECT b, '1' AS a FROM items"
    def replace_select_with_alias(*columns, &block)
      virtual_row_columns(columns, block)
      aliased_names = _aliased_column_names(columns)
      return self if !@opts[:select] || (@opts[:select] & aliased_names).empty?

      select(*(@opts[:select] - aliased_names + columns))
    end
    ...
    # Internal method to return the column names of alias expressions
    def _aliased_column_names(columns)
      columns.map do |column|
        case column.alias
        when nil
          next
        when Sequel::SQL::Identifier
          column.alias.value.to_sym
        else
          column.alias
        end
      end
