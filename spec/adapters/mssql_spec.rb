...
describe "Database#add_index with a condition" do
  before do
  ...
  context "on SQL Server 2005" do
    before { MSSQL_DB.stub(:server_version).and_return(9000000) }
    it "raises an error" do
      expect { MSSQL_DB.add_index(:items, :name, :where => {:deleted => 0}) }.to raise_error("Partial indexes are not supported for this database")
    end
  end

  context "on SQL Server 2008" do
    before { MSSQL_DB.stub(:server_version).and_return(10000000) }
    it "creates an index" do
      expect { MSSQL_DB.add_index(:items, :name, :where => {:deleted => 0}) }.to_not raise_error
      MSSQL_DB.indexes(:items).should have_key(:items_name_index)
    end
  end
end
