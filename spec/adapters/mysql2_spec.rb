...
unless defined?(MYSQL2_USER)
  MYSQL2_USER = 'root'
end
unless defined?(MYSQL2_DB)
  MYSQL2_URL = (ENV['SEQUEL_MY_SPEC_DB']||"mysql2://#{MYSQL2_USER}@localhost/sandbox") unless defined? MYSQL2_URL
  MYSQL2_DB = Sequel.connect(MYSQL2_URL)
end
unless defined?(MYSQL2_SOCKET_FILE)
  MYSQL2_SOCKET_FILE = '/tmp/mysql.sock'
end
INTEGRATION_DB = MYSQL2_DB unless defined?(INTEGRATION_DB)

MYSQL2_URI = URI.parse(MYSQL2_DB.uri)

MYSQL2_DB.create_table! :test2 do
  text :name
  integer :value
end
def MYSQL2_DB.sqls
...
MYSQL2_DB.loggers = [logger]
MYSQL2_DB.drop_table(:items) rescue nil
MYSQL2_DB.drop_table(:dolls) rescue nil
MYSQL2_DB.drop_table(:booltest) rescue nil

SQL_BEGIN = 'BEGIN'
SQL_ROLLBACK = 'ROLLBACK'
SQL_COMMIT = 'COMMIT'

context "Mysql2", '#create_table' do
  before do
    @db = MYSQL2_DB
    MYSQL2_DB.sqls.clear
  end
  after do
    @db.drop_table(:dolls) rescue nil
  end

  specify "should allow to specify options for Mysql2" do
...
context "A Mysql2 database" do
  specify "should provide the server version" do
    MYSQL2_DB.server_version.should >= 40000
  end

  specify "should handle the creation and dropping of an InnoDB table with foreign keys" do
    proc{MYSQL2_DB.create_table!(:test_innodb, :engine=>:InnoDB){primary_key :id; foreign_key :fk, :test_innodb, :key=>:id}}.should_not raise_error
  end

  specify "should support for_share" do
    MYSQL2_DB.transaction{MYSQL2_DB[:test2].for_share.all.should == []}
  end
end

if MYSQL2_DB.adapter_scheme == :mysql2
  context "Sequel::Mysql2.convert_tinyint_to_bool" do
    before do
      @db = MYSQL2_DB
    ...
    after do
      Sequel::Mysql2.convert_tinyint_to_bool = true
    ...
    specify "should consider tinyint(1) datatypes as boolean if set, but not larger tinyints" do
      ...
      Sequel::Mysql2.convert_tinyint_to_bool = false
...
context "A Mysql2 dataset" do
  before do
    MYSQL2_DB.create_table(:items){String :name; Integer :value}
    @d = MYSQL2_DB[:items]
    MYSQL2_DB.sqls.clear
  end
  after do
...
context "Mysql2 join expressions" do
  before do
    @ds = MYSQL2_DB[:nodes]
...
context "Joined Mysql2 dataset" do
  before do
    @ds = MYSQL2_DB[:nodes]
  end
...
context "A Mysql2 database with table options" do
  before do
    @options = {:engine=>'MyISAM', :charset=>'latin1', :collate => 'latin1_swedish_ci'}

    Sequel::Mysql2.default_engine = 'InnoDB'
    Sequel::Mysql2.default_charset = 'utf8'
    Sequel::Mysql2.default_collate = 'utf8_general_ci'
  ...
  after do
    @db.drop_table(:items) rescue nil
    Sequel::Mysql2.default_engine = nil
    Sequel::Mysql2.default_charset = nil
    Sequel::Mysql2.default_collate = nil
  end
...
if %w'localhost 127.0.0.1 ::1'.include?(MYSQL2_URI.host) and MYSQL2_DB.adapter_scheme == :mysql2
  context "A Mysql2 database" do
    specify "should accept a socket option" do
      db = Sequel.mysql2(MYSQL2_DB.opts[:database], :host => 'localhost', :user => MYSQL2_DB.opts[:user], :password => MYSQL2_DB.opts[:password], :socket => MYSQL2_SOCKET_FILE)
      proc {db.test_connection}.should_not raise_error
    end

    specify "should accept a socket option without host option" do
      db = Sequel.mysql2(MYSQL2_DB.opts[:database], :user => MYSQL2_DB.opts[:user], :password => MYSQL2_DB.opts[:password], :socket => MYSQL2_SOCKET_FILE)
      proc {db.test_connection}.should_not raise_error
    end

    specify "should fail to connect with invalid socket" do
      db = Sequel.mysql2(MYSQL2_DB.opts[:database], :user => MYSQL2_DB.opts[:user], :password => MYSQL2_DB.opts[:password], :socket =>'blah')
      proc {db.test_connection}.should raise_error
    end
  end
end

context "A grouped Mysql2 dataset" do
  before do
    MYSQL2_DB[:test2].delete
    MYSQL2_DB[:test2] << {:name => '11', :value => 10}
    MYSQL2_DB[:test2] << {:name => '11', :value => 20}
    MYSQL2_DB[:test2] << {:name => '11', :value => 30}
    MYSQL2_DB[:test2] << {:name => '12', :value => 10}
    MYSQL2_DB[:test2] << {:name => '12', :value => 20}
    MYSQL2_DB[:test2] << {:name => '13', :value => 10}
  end

  specify "should return the correct count for raw sql query" do
    ds = MYSQL2_DB["select name FROM test2 WHERE name = '11' GROUP BY name"]
    ds.count.should == 1
  end

  specify "should return the correct count for a normal dataset" do
    ds = MYSQL2_DB[:test2].select(:name).where(:name => '11').group(:name)
    ds.count.should == 1
  end
end
...
context "Mysql2::Dataset#insert and related methods" do
  before do
    MYSQL2_DB.create_table(:items){String :name; Integer :value}
    @d = MYSQL2_DB[:items]
    MYSQL2_DB.sqls.clear
  ...
  specify "#on_duplicate_key_update should work with regular inserts" do
    MYSQL2_DB.add_index :items, :name, :unique=>true
    MYSQL2_DB.sqls.clear
    ...
    MYSQL2_DB.sqls.length.should == 3
    MYSQL2_DB.sqls[0].should =~ /\AINSERT INTO items \((name|value), (name|value)\) VALUES \(('abc'|1), (1|'abc')\)\z/
    MYSQL2_DB.sqls[1].should =~ /\AINSERT INTO items \((name|value), (name|value)\) VALUES \(('abc'|1), (1|'abc')\) ON DUPLICATE KEY UPDATE name=VALUES\(name\), value=6\z/
    MYSQL2_DB.sqls[2].should =~ /\AINSERT INTO items \((name|value), (name|value)\) VALUES \(('def'|2), (2|'def')\) ON DUPLICATE KEY UPDATE name=VALUES\(name\), value=6\z/
  ...
  specify "#insert_ignore should add the IGNORE keyword for single inserts" do
    ...
    MYSQL2_DB.sqls.should == ["INSERT IGNORE INTO items (name) VALUES ('ghi')"]
...
context "Mysql2::Dataset#replace" do
  before do
    MYSQL2_DB.create_table(:items){Integer :id, :unique=>true; Integer :value}
    @d = MYSQL2_DB[:items]
    MYSQL2_DB.sqls.clear
  end
  after do
  ...
  specify "should use default values if they exist" do
    MYSQL2_DB.alter_table(:items){set_column_default :id, 1; set_column_default :value, 2}
    @d.replace
...
context "Mysql2::Dataset#complex_expression_sql" do
  before do
    @d = MYSQL2_DB.dataset
  end
