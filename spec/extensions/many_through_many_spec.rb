...
describe Sequel::Model, "many_through_many" do
  ...
  it "should allowing filtering by many_through_many associations with :limit with :lateral_subquery limit strategy" do
    @c1.many_through_many :tags, [[:albums_artists, :artist_id, :album_id], [:albums, :id, :id], [:albums_tags, :album_id, :tag_id]], :limit=>10, :filter_limit_strategy=>:lateral_subquery
    @c1.filter(:tags=>@c2.load(:id=>1234)).sql.must_equal "SELECT * FROM artists WHERE (artists.id IN (SELECT artists.id FROM artists INNER JOIN LATERAL (SELECT tags.id FROM tags INNER JOIN albums_tags ON (albums_tags.tag_id = tags.id) INNER JOIN albums ON (albums.id = albums_tags.album_id) INNER JOIN albums_artists ON (albums_artists.album_id = albums.id) WHERE (albums_artists.artist_id = artists.id) LIMIT 10) AS tags ON (tags.id = 1234) WHERE (artists.id IN (SELECT albums_artists.artist_id FROM albums_artists INNER JOIN albums ON (albums.id = albums_artists.album_id) INNER JOIN albums_tags ON (albums_tags.album_id = albums.id) WHERE (albums_tags.tag_id = 1234)))))"
  end

  it "should allowing filtering by many_through_many associations with :limit and composite keys with :lateral_subquery limit strategy" do
    @c1.many_through_many :tags, [[:albums_artists, [:b1, :b2], [:c1, :c2]], [:albums, [:d1, :d2], [:e1, :e2]], [:albums_tags, [:f1, :f2], [:g1, :g2]]], :right_primary_key=>[:h1, :h2], :left_primary_key=>[:id, :yyy], :limit=>10, :filter_limit_strategy=>:lateral_subquery
    @c1.filter(:tags=>[@c2.load(:id=>1, :h1=>1234, :h2=>85)]).sql.must_equal "SELECT * FROM artists WHERE ((artists.id, artists.yyy) IN (SELECT artists.id, artists.yyy FROM artists INNER JOIN LATERAL (SELECT tags.id FROM tags INNER JOIN albums_tags ON ((albums_tags.g1 = tags.h1) AND (albums_tags.g2 = tags.h2)) INNER JOIN albums ON ((albums.e1 = albums_tags.f1) AND (albums.e2 = albums_tags.f2)) INNER JOIN albums_artists ON ((albums_artists.c1 = albums.d1) AND (albums_artists.c2 = albums.d2)) WHERE ((albums_artists.b1 = artists.id) AND (albums_artists.b2 = artists.yyy)) LIMIT 10) AS tags ON (tags.id IN (1)) WHERE ((artists.id, artists.yyy) IN (SELECT albums_artists.b1, albums_artists.b2 FROM albums_artists INNER JOIN albums ON ((albums.d1 = albums_artists.c1) AND (albums.d2 = albums_artists.c2)) INNER JOIN albums_tags ON ((albums_tags.f1 = albums.e1) AND (albums_tags.f2 = albums.e2)) WHERE ((albums_tags.g1, albums_tags.g2) IN ((1234, 85)))))))"
  end

  it "should allowing filtering by many_through_many associations with :limit and :conditions with :lateral_subquery limit strategy" do
    @c1.many_through_many :tags, [[:albums_artists, :artist_id, :album_id], [:albums, :id, :id], [:albums_tags, :album_id, :tag_id]], :conditions=>{:name=>'A'}, :limit=>10, :filter_limit_strategy=>:lateral_subquery
    @c1.filter(:tags=>[@c2.load(:id=>1234)]).sql.must_equal "SELECT * FROM artists WHERE (artists.id IN (SELECT artists.id FROM artists INNER JOIN LATERAL (SELECT tags.id FROM tags INNER JOIN albums_tags ON (albums_tags.tag_id = tags.id) INNER JOIN albums ON (albums.id = albums_tags.album_id) INNER JOIN albums_artists ON (albums_artists.album_id = albums.id) WHERE ((name = 'A') AND (albums_artists.artist_id = artists.id)) LIMIT 10) AS tags ON (tags.id IN (1234)) WHERE (artists.id IN (SELECT albums_artists.artist_id FROM albums_artists INNER JOIN albums ON (albums.id = albums_artists.album_id) INNER JOIN albums_tags ON (albums_tags.album_id = albums.id) WHERE (albums_tags.tag_id IN (1234))))))"
  end

  it "should allowing filtering by many_through_many associations with :limit and :conditions and composite keys with :lateral_subquery limit strategy" do
    @c1.many_through_many :tags, [[:albums_artists, [:b1, :b2], [:c1, :c2]], [:albums, [:d1, :d2], [:e1, :e2]], [:albums_tags, [:f1, :f2], [:g1, :g2]]], :right_primary_key=>[:h1, :h2], :left_primary_key=>[:id, :yyy], :conditions=>{:name=>'A'}, :limit=>10, :filter_limit_strategy=>:lateral_subquery
    @c1.filter(:tags=>@c2.load(:id=>1, :h1=>1234, :h2=>85)).sql.must_equal "SELECT * FROM artists WHERE ((artists.id, artists.yyy) IN (SELECT artists.id, artists.yyy FROM artists INNER JOIN LATERAL (SELECT tags.id FROM tags INNER JOIN albums_tags ON ((albums_tags.g1 = tags.h1) AND (albums_tags.g2 = tags.h2)) INNER JOIN albums ON ((albums.e1 = albums_tags.f1) AND (albums.e2 = albums_tags.f2)) INNER JOIN albums_artists ON ((albums_artists.c1 = albums.d1) AND (albums_artists.c2 = albums.d2)) WHERE ((name = 'A') AND (albums_artists.b1 = artists.id) AND (albums_artists.b2 = artists.yyy)) LIMIT 10) AS tags ON (tags.id = 1) WHERE ((artists.id, artists.yyy) IN (SELECT albums_artists.b1, albums_artists.b2 FROM albums_artists INNER JOIN albums ON ((albums.d1 = albums_artists.c1) AND (albums.d2 = albums_artists.c2)) INNER JOIN albums_tags ON ((albums_tags.f1 = albums.e1) AND (albums_tags.f2 = albums.e2)) WHERE ((albums_tags.g1 = 1234) AND (albums_tags.g2 = 85))))))"
  end

  it "should allowing filtering by one_through_many associations with :limit with :lateral_subquery limit strategy and a dataset" do
    @c1.one_through_many :tag, [[:albums_artists, :artist_id, :album_id], [:albums, :id, :id], [:albums_tags, :album_id, :tag_id]], :filter_limit_strategy=>:lateral_subquery
    @c1.filter(:tag=>@c2.where(:x=>1234)).sql.must_equal "SELECT * FROM artists WHERE (artists.id IN (SELECT artists.id FROM artists INNER JOIN LATERAL (SELECT tags.id FROM tags INNER JOIN albums_tags ON (albums_tags.tag_id = tags.id) INNER JOIN albums ON (albums.id = albums_tags.album_id) INNER JOIN albums_artists ON (albums_artists.album_id = albums.id) WHERE (albums_artists.artist_id = artists.id) LIMIT 1) AS tags ON (tags.id IN (SELECT tags.id FROM tags WHERE (x = 1234))) WHERE (artists.id IN (SELECT albums_artists.artist_id FROM albums_artists INNER JOIN albums ON (albums.id = albums_artists.album_id) INNER JOIN albums_tags ON (albums_tags.album_id = albums.id) WHERE (albums_tags.tag_id IN (SELECT tags.id FROM tags WHERE (x = 1234)))))))"
  end
...
describe "many_through_many/one_through_many associations with :db option" do
  before do
    ...
    @db3.fetch = {:foo_id=>444}
    @db4.fetch = {:attribute_id=>555}
    @db1.fetch = {:id=>555}
    @through = [{:table=>:foo_nodes, :left=>:node_id, :right=>:foo_id, :db=>@db3},
      {:table=>:bar_attributes, :left=>:bar_id, :right=>:attribute_id, :db=>@db4}]
    @composite_through = [@through[0].merge(:left=>[:l1, :l2], :right=>[:ml1, :ml2]),
      @through[1].merge(:left=>[:mr1, :mr2], :right=>[:r1, :r2])]
  ...
  it "should support dataset method" do
    ...
    sqls.must_equal [[], [], ["SELECT foo_id FROM foo_nodes WHERE (node_id = 1234)"],
      ["SELECT attribute_id FROM bar_attributes WHERE (bar_id IN (444))"]]
  end
  ...
  it "should support association method" do
    ...
    sqls.must_equal [["SELECT attributes.* FROM attributes WHERE (id IN (555))"], [],
      ["SELECT foo_id FROM foo_nodes WHERE (node_id = 1234)"],
      ["SELECT attribute_id FROM bar_attributes WHERE (bar_id IN (444))"]]

    @c2.one_through_many :attribute, @through, :class => @c1 
  ...
  it "should support a conditions option" do
    @c2.many_through_many :attributes, @through, :class => @c1, :conditions => {:a=>32}
    @c2.new(:id => 1234).attributes.must_equal [@c1.load(:id=>555)]
  ...
  it "should support an order option" do
    @c2.many_through_many :attributes, @through, :class => @c1, :order=>:blah
    @c2.new(:id => 1234).attributes.must_equal [@c1.load(:id=>555)]
  ...
  it "should support a conditions option in join table" do
    @through[0][:conditions] = {:b=>3}
    @through[1][:conditions] = {:c=>4}
    @c2.many_through_many :attributes, @through, :class => @c1
    @c2.new(:id => 1234).attributes.must_equal [@c1.load(:id=>555)]
    sqls.must_equal [["SELECT attributes.* FROM attributes WHERE (id IN (555))"], [],
      ["SELECT foo_id FROM foo_nodes WHERE ((node_id = 1234) AND (b = 3))"],
      ["SELECT attribute_id FROM bar_attributes WHERE ((bar_id IN (444)) AND (c = 4))"]]
  end
  
  it "should support :left_primary_key and :right_primary_key options" do
    @c2.many_through_many :attributes, @through, :class => @c1, :left_primary_key=>:xxx, :right_primary_key=>:yyy
    @db3.fetch = {:foo_id=>7}
    @db4.fetch = {:attribute_id=>555}
    ...
    sqls.must_equal [["SELECT attributes.* FROM attributes WHERE (yyy IN (555))"], [],
      ["SELECT foo_id FROM foo_nodes WHERE (node_id = 5)"],
      ["SELECT attribute_id FROM bar_attributes WHERE (bar_id IN (7))"]]
  end
  
  it "should support composite keys" do
    @c2.many_through_many :attributes, @composite_through, :class => @c1, :left_primary_key=>[:id, :x], :right_primary_key=>[:id, :y]
    @db3.fetch = {:ml1=>17, :ml2=>18}
    @db4.fetch = {:r1=>14, :r2=>555}
    ...
    sqls.must_equal [["SELECT attributes.* FROM attributes WHERE ((id, y) IN ((14, 555)))"], [],
      ["SELECT ml1, ml2 FROM foo_nodes WHERE ((l1, l2) IN ((1234, 5)))"],
      ["SELECT r1, r2 FROM bar_attributes WHERE ((mr1, mr2) IN ((17, 18)))"]]
  end
  ...
  it "should handle case where join table query does not produce any rows" do
    ...
    sqls.must_equal [[], [], ["SELECT ml1, ml2 FROM foo_nodes WHERE ((l1, l2) IN ((1234, 5)))"], []]
  end
  ...
  it "should handle case where join table query returns a NULL value" do
    ...
    @c2.many_through_many :attributes, @through, :class => @c1
    @db4.fetch = {:attribute_id=>nil}
    @c2.new(:id => 1234).attributes.must_equal []
    sqls.must_equal [[], [], ["SELECT foo_id FROM foo_nodes WHERE (node_id = 1234)"],
      ["SELECT attribute_id FROM bar_attributes WHERE (bar_id IN (444))"]]

    @db3.fetch = {:foo_id=>nil}
    @c2.new(:id => 1234).attributes.must_equal []
    sqls.must_equal [[], [], ["SELECT foo_id FROM foo_nodes WHERE (node_id = 1234)"],
      []]
    ...
    @c2.many_through_many :attributes, @composite_through, :class => @c1, :left_primary_key=>[:id, :x], :right_primary_key=>[:id, :y]
    @db3.fetch = {:ml1=>17, :ml2=>18}
    @db4.fetch = [{:r1=>14, :r2=>nil}, {:r1=>nil, :r2=>555}, {:r1=>nil, :r2=>nil}]
    @c2.load(:id => 1234, :x=>5).attributes.must_equal []
    sqls.must_equal [[], [], ["SELECT ml1, ml2 FROM foo_nodes WHERE ((l1, l2) IN ((1234, 5)))"],
      ["SELECT r1, r2 FROM bar_attributes WHERE ((mr1, mr2) IN ((17, 18)))"]]

    @db3.fetch = [{:ml1=>17, :ml2=>nil}, {:ml1=>17, :ml2=>nil}, {:ml1=>nil, :ml2=>nil}]
    @c2.load(:id => 1234, :x=>5).attributes.must_equal []
    sqls.must_equal [[], [], ["SELECT ml1, ml2 FROM foo_nodes WHERE ((l1, l2) IN ((1234, 5)))"], []]
  end
  
  it "should support a select option" do
    @c2.many_through_many :attributes, @through, :class => @c1, :select => :blah
    @db1.fetch = {:blah=>19}
    @c2.load(:id => 1234, :x=>5).attributes.must_equal [@c1.load(:blah=>19)]
  ...
  it "should accept a block" do
    @c2.many_through_many :attributes, @through, :class => @c1 do |ds|
      ds.filter(:xxx => @xxx)
    end
  ...
  it "should handle an aliased join table" do
    @through[0][:table] = Sequel[:foo_nodes].as(:fn)
    ...
    sqls.must_equal [["SELECT attributes.* FROM attributes WHERE (id IN (555))"], [],
      ...
      ["SELECT attribute_id FROM bar_attributes AS ba WHERE (bar_id IN (444))"]]
  end
  ...
  it "should support eager loading" do
    ...
    sqls.must_equal [["SELECT attributes.* FROM attributes WHERE (id IN (555))"],
      ["SELECT * FROM nodes"],
      ["SELECT foo_id, node_id FROM foo_nodes WHERE (node_id IN (1234, 33))"],
  ...
  it "should support eager loading when the join table includes NULL values" do
    @db2.fetch = [{:id=>1234}, {:id=>33}]
    @db3.fetch = {:foo_id=>444, :node_id=>1234}
    @db4.fetch = {:attribute_id=>nil, :bar_id=>444}
    ...
    @db3.fetch = {:foo_id=>nil, :node_id=>1234}
    a = @c2.eager(:attributes).all
    a.must_equal [@c2.load(:id=>1234), @c2.load(:id=>33)]
    a[0].associations[:attributes].must_equal []
    a[1].associations[:attributes].must_equal []
  ...
  it "should support eager loading when using composite keys" do
    ...
    @db3.fetch = {:ml1=>17, :ml2=>18, :l1=>1234, :l2=>333}
    @db4.fetch = {:r1=>14, :r2=>555, :mr1=>17, :mr2=>18}
    ...
    sqls.must_equal [["SELECT attributes.* FROM attributes WHERE ((id, y) IN ((14, 555)))"],
      ["SELECT * FROM nodes"],
      ["SELECT ml1, ml2, l1, l2 FROM foo_nodes WHERE ((l1, l2) IN ((1234, 333), (33, 4)))"],
      ["SELECT r1, r2, mr1, mr2 FROM bar_attributes WHERE ((mr1, mr2) IN ((17, 18)))"]]
  ...
  it "should support eager loading when using composite keys when the join table includes NULL values" do
    ...
    @db2.fetch = [{:id=>1234, :x=>333}, {:id=>33, :x=>4}]
    @db3.fetch = {:ml1=>17, :ml2=>18, :l1=>1234, :l2=>333}
    @db4.fetch = [{:r1=>nil, :r2=>555, :mr1=>17, :mr2=>18}, {:r1=>14, :r2=>nil, :mr1=>17, :mr2=>18}, {:r1=>nil, :r2=>nil, :mr1=>17, :mr2=>18}]
    ...
    @db3.fetch = [{:ml1=>nil, :ml2=>18, :l1=>1234, :l2=>333}, {:ml1=>17, :ml2=>nil, :l1=>1234, :l2=>333}, {:ml1=>nil, :ml2=>nil, :l1=>1234, :l2=>333}]
    @c2.many_through_many :attributes, @composite_through, :class => @c1, :left_primary_key=>[:id, :x], :right_primary_key=>[:id, :y]
    a = @c2.eager(:attributes).all
    a.must_equal [@c2.load(:id=>1234, :x=>333), @c2.load(:id=>33, :x=>4)]
    a[0].associations[:attributes].must_equal []
