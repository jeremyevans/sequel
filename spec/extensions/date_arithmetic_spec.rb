...
describe "date_arithmetic extension" do
  ...
  before do
    ...
    @h2 = {:years=>1, :months=>1, :weeks=>1, :days=>1, :hours=>1, :minutes=>1, :seconds=>1}
  end
  ...
  it "should raise an error if given invalid keys in an interval hash" do
    lambda{Sequel.date_add(:a, :days=>1, :foo=>1)}.must_raise(Sequel::Error)
  end
  ...
  if asd
    it "should have Sequel.date_add with an ActiveSupport::Duration return an appropriate Sequel::SQL::DateAdd expression" do
      ...
      Sequel.date_add(:a, ActiveSupport::Duration.new(1, [[:weeks, 1]])).interval.must_equal(:days=>7)
    end
    ...
    it "should have Sequel.date_sub with an ActiveSupport::Duration return an appropriate Sequel::SQL::DateAdd expression" do
      ...
      Sequel.date_sub(:a, ActiveSupport::Duration.new(1, [[:weeks, 1]])).interval.must_equal(:days=>-7)
    end
  end
  ...
  it "should use existing method" do
    ...
    db.literal(Sequel.date_add(:a, @h2)).must_equal "da(a, '1 years 1 months 8 days 1 hours 1 minutes 1 seconds ')"
  end
  ...
  it "should correctly literalize on SQLite" do
    ...
    db.literal(Sequel.date_add(:a, @h2)).must_equal "datetime(a, '1 years', '1 months', '8 days', '1 hours', '1 minutes', '1 seconds')"
  end
  ...
  it "should correctly literalize on MySQL" do
    ...
    db.literal(Sequel.date_add(:a, @h2)).must_equal "DATE_ADD(DATE_ADD(DATE_ADD(DATE_ADD(DATE_ADD(DATE_ADD(a, INTERVAL 1 YEAR), INTERVAL 1 MONTH), INTERVAL 8 DAY), INTERVAL 1 HOUR), INTERVAL 1 MINUTE), INTERVAL 1 SECOND)"
  ...
  it "should correctly literalize on HSQLDB" do
    ...
    db.literal(Sequel.date_add(:a, @h2)).must_equal "DATE_ADD(DATE_ADD(DATE_ADD(DATE_ADD(DATE_ADD(DATE_ADD(CAST(a AS timestamp), INTERVAL 1 YEAR), INTERVAL 1 MONTH), INTERVAL 8 DAY), INTERVAL 1 HOUR), INTERVAL 1 MINUTE), INTERVAL 1 SECOND)"

    db.literal(Sequel.date_add(:a, @h0, :cast=>:datetime)).must_equal "CAST(CAST(a AS datetime) AS datetime)"
    db.literal(Sequel.date_add(:a, @h2, :cast=>:datetime)).must_equal "DATE_ADD(DATE_ADD(DATE_ADD(DATE_ADD(DATE_ADD(DATE_ADD(CAST(a AS datetime), INTERVAL 1 YEAR), INTERVAL 1 MONTH), INTERVAL 8 DAY), INTERVAL 1 HOUR), INTERVAL 1 MINUTE), INTERVAL 1 SECOND)"
  end
  ...
  it "should correctly literalize on MSSQL" do
    ...
    db.literal(Sequel.date_add(:A, @h2)).must_equal "DATEADD(second, 1, DATEADD(minute, 1, DATEADD(hour, 1, DATEADD(day, 8, DATEADD(month, 1, DATEADD(year, 1, A))))))"
    db.literal(Sequel.date_add(:A, @h0, :cast=>:timestamp)).must_equal "CAST(A AS timestamp)"
  end
  ...
  it "should correctly literalize on access" do
    ...
    db.literal(Sequel.date_add(:a, @h2)).must_equal "DATEADD('s', 1, DATEADD('n', 1, DATEADD('h', 1, DATEADD('d', 8, DATEADD('m', 1, DATEADD('yyyy', 1, a))))))"
  end
  ...
  it "should correctly literalize on Derby" do
    ...
    db.literal(Sequel.date_add(:a, @h2)).must_equal "{fn timestampadd(SQL_TSI_SECOND, 1, timestamp({fn timestampadd(SQL_TSI_MINUTE, 1, timestamp({fn timestampadd(SQL_TSI_HOUR, 1, timestamp({fn timestampadd(SQL_TSI_DAY, 8, timestamp({fn timestampadd(SQL_TSI_MONTH, 1, timestamp({fn timestampadd(SQL_TSI_YEAR, 1, timestamp(a))}))}))}))}))}))}"
  ...
  it "should correctly literalize on Oracle" do
    ...
    db.literal(Sequel.date_add(:A, @h2)).must_equal "(A + INTERVAL '1' YEAR + INTERVAL '1' MONTH + INTERVAL '8' DAY + INTERVAL '1' HOUR + INTERVAL '1' MINUTE + INTERVAL '1' SECOND)"
    db.literal(Sequel.date_add(:A, @h0, :cast=>:datetime)).must_equal "CAST(A AS datetime)"
  end
  ...
  it "should correctly literalize on DB2" do
    ...
    db.literal(Sequel.date_add(:A, @h2, :cast=>:datetime)).must_equal "(CAST(A AS datetime) + 1 years + 1 months + 8 days + 1 hours + 1 minutes + 1 seconds)"
  end
