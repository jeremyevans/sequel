...
describe "Sequel core extension replacements" do
  ...
  it "Sequel::SQL::Identifier where tbale is Sequel::SQL::Identifier wrapping literal string should not quote table" do
    ...
    l(Sequel.identifier('t')[:c], '"t"."c"')
    l(Sequel.identifier(Sequel.lit('t'))[:c], 't."c"')
  end
...
describe "Sequel.synchronize_with" do
  it "should yield if given nil" do
    Sequel.synchronize_with(nil){1}.must_equal 1
  end

  it "should synchronize with given mutex" do
    m = Mutex.new
    Sequel.synchronize_with(m){1}.must_equal 1

    proc do
      Sequel.synchronize_with(m) do
        Sequel.synchronize_with(m){}
      end
