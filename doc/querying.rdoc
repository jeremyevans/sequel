...
<tt>Sequel::Model.[]</tt> is the shortest method to use to find a model instance
...
<tt>Sequel::Model.[]</tt> also supports passing a hash for non-primary key querying. If you
want a method that always does a primary key lookup, you can use <tt>Sequel::Model.with_pk</tt>:

  artist = Artist.with_pk(1)

For both methods, if is no record with the given primary key, +nil+ will be returned. If you want
...
For arrays and sets, Sequel uses the IN operator with a value list:

  Artist.where(id: [1, 2])
  # SELECT * FROM artists WHERE (id IN (1, 2))

  Artist.where(id: Set[1, 2])
  # SELECT * FROM artists WHERE (id IN (1, 2))
...
The SQL standard defines a few helpful methods to get aggregate information about
