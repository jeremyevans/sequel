...
transactions around entire test suites, using savepoints around each test.  This can significantly
...
SEQUEL_LIT_REQUIRE_FROZEN :: Use the lit_require_frozen extension when running the specs
SEQUEL_MODEL_PREPARED_STATEMENTS :: Use the prepared_statements plugin when running the specs
SEQUEL_MODEL_SHAPE_FRIENDLY :: Set Sequel::Model.shape_friendly to true when running the specs
...
SEQUEL_SELECT_ON_SKIPPED_UPDATE :: Use the select_on_skipped_update plugin when running the adapter and integration specs
