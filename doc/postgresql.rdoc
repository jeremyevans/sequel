...
With regard to common database types, please note that String columns will
default to PostgreSQL's <tt>text</tt> type, with an unbounded size.  You must
specify a <tt>:size</tt> option to get a <tt>varchar</tt> type and impose a
size constraint on the column.
