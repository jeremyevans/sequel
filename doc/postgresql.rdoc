...
=== Creating/Altering/Dropping Property Graphs (PostgreSQL 19+)

Sequel supports +create_property_graph+, +alter_property_graph+, +rename_property_graph+, +set_property_graph_schema+, and +drop_property_graph+ for creating, altering, and dropping property graphs:
  ...
  DB.drop_property_graph(Sequel[:other_schema][:g2])
  # DROP PROPERTY GRAPH other_schema.g2
...
=== GRAPH_TABLE support (PostgresSQL 19+)

+graph_table+ is supported on PostgreSQL Database objects. It returns a
Sequel::PropertyGraph::Table object that uses a method chaining approach to
build the GRAPH_TABLE expression, similarly to how Datasets are created via
method chaining:
...
=== Foreign Key Constraints Referencing A Given Table
...
You can get a array of the Database's property graphs using the +property_graphs+ method:
