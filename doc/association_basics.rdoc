...
Note that for the final two associations, you didn't have to specify the :class,
...
:lateral_subquery :: Uses a LATERAL subquery to ensure the correctly
                     limited/offset records are returned.
...
Sequel will choose either a :distinct_on, :window_function,
:correlated_subquery, or :lateral_subquery strategy based on the association
type and what the database supports, but you can override that if necessary using
this option (or the :eager_limit_strategy option).
