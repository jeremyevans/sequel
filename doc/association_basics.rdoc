...
Starting in Sequel 4.19.0, you can override how Sequel gets column values
by overriding <tt>Sequel::Model#get_column_value</tt>.  By overriding it
to not call +send+ for column names that conflict that existing method names,
you can avoid these issues.

The rest of the section documents the previous way to work around this issue,
which also can be used to base associations on arbitrary SQL expressions.
