...
(Under any circumstances, it is critical that you save the result of the Sequel.connect in a variable. Sequel caches the connecting in an internal data structure, so if you just do Sequel.connect many times - like once for every incoming http request - your application will leak huge amounts of memory. Another way to avoid this is to use the block form as described further down in this guide.)
