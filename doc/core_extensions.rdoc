...
Most of the these extensions can be added on a per-file basis using refinements.  To use refinements, first load them:
...
Array#sql_or and Hash#sql_or treat the receiver as a conditions specifier, matching any of the conditions in the array:
...
Array#sql_string_join joins all of the elements in the array in an SQL string concatenation expression:
