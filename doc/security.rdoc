...
==== ComplexExpression#extract field

Sequel converts the field to extract to a string and uses it directly.

  DB[:table].select(Sequel[:column].extract(params[:field])) # SQL injection!
  DB[:table].select(Sequel.extract(params[:field], :column)) # SQL injection!
...
so much easier, it is recommended to not use it, and instead
...
Sequel::SQL::Identifier#[][rdoc-ref:Sequel::SQL::QualifyingMethods#[]] for qualified identifiers, and
