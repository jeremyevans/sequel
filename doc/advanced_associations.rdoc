...
If the database supports window functions, you can use the :window_function strategy:
...
If the database supports LATERAL subqueries, you can use the :lateral_subquery strategy:

  Artist.one_to_many :first_10_albums, class: :Album, order: :release_date, limit: 10,
    eager_limit_strategy: :lateral_subquery
  # SELECT *
  # FROM albums
  # WHERE (albums.id IN (
  #   SELECT albums.id
...
By default Sequel retrieves all records and then does the array slice in ruby.
As eager_graph does not support options, to use an eager_graph limit strategy
you have to use the eager_graph_with_options method with the :limit_strategy option.

The eager_graph limit strategies Sequel supports are described below.
...
subquery (only supported for one_to_many/one_to_one):
...
The :lateral_subquery uses a LATERAL JOIN to a subquery, and moves the conditions
for the JOIN into the subquery, as that is required on some/all databases to make the query
fast (only supported for one_to_many/one_to_one):

  Artist.eager_graph_with_options(:first_10_albums, limit_strategy: :lateral_subquery).all
  # SELECT artists.id, artists.name, first_10_albums.id AS first_10_albums_id,
  #        first_10_albums.name AS first_10_albums_name, first_10_albums.artist_id,
  #        first_10_albums.release_date
  # FROM artists 
...
return a few of many records, it may be cheaper to get all of their associated records and filter
them in ruby as opposed to computing the set of limited associated records for all rows.

Sequel may in the future make the :lateral_subquery limit strategy the default strategy
on the databases that support it.
...
it's just because those are the simplest examples.  Sequel supports most of the same features for
    ...
    many_to_one :ticket_hours, read_only: true, key: :id,
     ...
     eager_loader: (lambda do |eo|
      ...
      Ticket.where(project_id: eo[:id_map].keys).
       ...
       all do |t|
        p = eo[:id_map][t.remove_key!(:project_id)].first
